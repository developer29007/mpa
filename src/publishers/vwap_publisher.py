...
from util.TimeUtil import ms_to_nanos, nanos_to_us_str
...
class VwapPublisher(TradeListener, TimerListener, KafkaPublisher):
    ...
    def on_trade(self, trade: Trade):
        ...
        if not self._timer_registered:
            # print(f"[REGISTER] {self._interval_ms}ms timer at {nanos_to_us_str(trade.timestamp_ns)}")
    ...
    def on_timer_expired(self, time_interval: int, scheduled_time: int, time_now: int):
        ...
        # print(f"[TIMER] {time_interval // 1_000_000}ms fired at {nanos_to_us_str(publish_time)}")
