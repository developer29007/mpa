...
def test_deserialize_trade():
    payload = struct.pack(
        TRADE_FORMAT,
        42,                                  # msg_seq
        36_000_000_000_000,                  # ts_me (10:00:00)
        b"AAPL    ",                         # sec_id
        100,                                 # shares
        150.25,                              # price
        b"B",                                # side
    ...
    assert result["ts_me"] == 36_000_000_000_000
    assert result["sec_id"] == "AAPL"
    assert result["exch_id"] == "XNAS"
    assert result["src_id"] == "itch"
...
def test_deserialize_vwap():
    ...
    assert result["msg_seq"] == 7
    assert result["ts_me"] == 36_000_000_000_000
    assert result["sec_id"] == "MSFT"
...
def test_deserialize_tob():
    payload = struct.pack(
        ...
        35_999_500_000_000,                  # last_trade_ts_me
    ...
    assert result["msg_seq"] == 10
    assert result["ts_me"] == 36_000_000_000_000
    assert result["sec_id"] == "TSLA"
    ...
    assert result["last_trade_ts_me"] == 35_999_500_000_000
